...
export default function EventsAdminPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const data = { ...formData, date: formData.date ? new Date(formData.date).toISOString() : null, endDate: formData.endDate ? new Date(formData.endDate).toISOString() : null };
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div><Label>Date début</Label><Input type="datetime-local" value={formData.date || ''} onChange={(e) => setFormData({ ...formData, date: e.target.value })} required /></div>
                <div><Label>Date fin</Label><Input type="datetime-local" value={formData.endDate || ''} onChange={(e) => setFormData({ ...formData, endDate: e.target.value })} /></div>
              </div>
            ...
            {events.map((item: Event) => (
              <div key={item.id} className="flex items-start gap-4 p-4 bg-card border rounded-lg">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-muted-foreground">{item.location} • {format(new Date(item.date), 'dd/MM/yyyy HH:mm')}</p>
                </div>
